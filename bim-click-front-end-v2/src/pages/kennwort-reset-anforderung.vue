<!-- kennwort-reset-anforderung.vue -->
<!-- Vue/Quasar-Komponente zur Anforderung eines Reset des Kennworts des per E-Mail-Adresse identifizierten Benuterkontos -->
<!-- Die dazu passende E-Mail-Adresse muss vom Benutzer eingegeben werden -->
    ...
    <div class="fixed-center text-center">
      <q-card class="my-card-anzeige q-pa-sm">
        ...
        <q-banner rounded dense class="bg-cyan-7 text-white text-h4">
          Wenn Sie das Kennwort für Ihr Benutzerkonto bei BIM.click vergessen
          haben oder aus einem anderen Grund neu vergeben wollen, geben Sie im
          Eingabefeld unten die E-Mail-Adresse ein, mit der Sie sich
          als Benutzerkennung für BIM.click registriert haben. Drücken Sie dann
          die Schaltfläche darunter ("Neuvergabe des Kennworts für die
          eingegebene E-Mail-Adresse veranlasssen"). Sie erhalten dann eine
        ...
        <q-input
          ...
          debounce="300"
          color="primary"
          hint="Bitte in obigem Feld die E-Mail-Adresse eingeben, mit der Sie sich als Benutzerkennung für BIM.click registriert haben. Dann die Schaltfläche unten betätigen."
        ...
        <q-btn
          ...
          label="Neuvergabe des Kennworts für die eingegebene E-Mail-Adresse veranlassen"
          no-caps
          color="positive"
          glossy
          size="md"
          @click="submitRequestResetKennWortBenutzer()"
        >
        </q-btn>
      </q-card>
    </div>
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
export default {
  name: "KennWortResetAnforderung", // Vue Component Name - ACHTUNG - dabei beachten:
  ...
  //----------------------------------------------------------------------------------------------------------------------------------------------------
  methods: {
    submitRequestResetKennWortBenutzer: function() {
      // Per Axios die neue Post-Methode requestResetKennWortBenutzer am Backend aurufen zur Auslösung eines Kennwort-Resets für die E-Mail-Adresse des Benutzers
      // Im Request Body muss die E-Mail-Adresse mitgegeben werden
      // Die E-Mail-Adresse kommt aus obigem Input Feld (Variable eMail)
      // HINWEIS: Hier wird noch kein Reset Token verwendet - dieser wird in requestResetKennWortBenutzer erst erzeugt und mit einer
      //          E-Mail an den Benutzer in einem Link mit verschickt. Er wird dann im nachfolgenden Schritt in der Frontend Route
      //          /Authentifizierung/Kennwortreset/:token
      //          wenn der eigentliche Reset durchgeführt wird (resetKBenutzer, in kennwort-reset.vue)
      const apiRoute =
        process.env.API + "/Authentifizierung/Kennwortreset/Anforderung";
      this.$axios
        .post(apiRoute, {
          // Body:
        ...
        .then(response => {
          ...
          globalStore.showBenutzerNachricht(
            BenutzerNachrichten.AUTHORISIERUNG_KENNWORT_RESET_ANFORDERUNG_ERFOLG
          );
        ...
        .catch(error => {
          ...
          //------------------------------------------------------------------------------------------------------------------------------
          globalStore.showBenutzerNachricht(
            BenutzerNachrichten.AUTHORISIERUNG_KENNWORT_RESET_ANFORDERUNG_FEHLER
          );
        });
    }
  }
