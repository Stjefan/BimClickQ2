<template>
  <div>
    <q-table
      ...
      v-bind:class="{ stickyColumnsTable1HeaderLineBauinformationssProfile: $q.screen.gt.sm }"
      >
...
export default {
  ...
  props: {
    ...
    dataTableBauinformationsProfile: Array,
    columnsTableBauinformationsProfile: Array,
  ...
  data() {
    return {
      ...
      internalSelectedRowsTableBauinformationsProfile: null, // this.selected !== null ? [this.selected] : [],
      filterTableBauinformationsProfile: null,
      paginationTableBauinformationsProfile: {
        sortBy: "bauinformationsProfilBezeichnung", // HINWEIS: Hier stand ursprünglich nur "Name"!!!
        ...
        // unter Verwendung von selectedNodeTreeBaustoffe.bauinformationen.length, wenn dieses initialisiert und geändert wird
      }
    };
  },
    ...
    contextMenuSelectionHandler(goToTab, row) {
      ...
      switch (goToTab) {
        case "Bearbeitung":
          ...
          this.internalSelectedRowsTableBauinformationsProfile.push(row);
          // HINWEISE zur Event-Verarbeitung:
          // Die nächsten Events benachrichtigen die Elternkomponente bauinformations-profile.vue
          // a) dass das ausgewählte Bauinformationsprofil in der Table für die Profile geändert wurde
          // b) dass sie zum  Details-Tab (per Parameter goToTab) für das neu ausgewählte Profil wechseln soll
          // ACHTUNG: Die Komponente bauinformations-profile-tab-baustoffe-und-baueinheiten.vue verwendet für die Übersichts-Table der
          //          Profile NICHT diese Kindkomponente hier, sondern implementiert selbst die QTable!!!
          this.$emit(
            "selection-changed-table-bauinformations-profile-bauinformations-profil",
            this.internalSelectedRowsTableBauinformationsProfile
          );
          this.$emit("change-tab-request-bauinformations-profil", goToTab);
          break;
        case "Neuanlage":
          this.$emit("change-tab-request-bauinformations-profil", goToTab);
          break;
        case "Löschen":
          ...
          // Die nächsten Events benachrichtigen die Elternkomponente bauinformations-profile.vue
          // a) dass das ausgewählte Bauinformationsprofil in der Table für die Profile geändert wurde
          // b) dass sie das ausgewählte Bauinformationsprofil löschen soll
          // ACHTUNG: Die Komponente bauinformations-profile-tab-baustoffe-und-baueinheiten.vue verwendet für die Übersichts-Table der
          //          Profile NICHT diese Kindkomponente hier, sondern implementiert selbst die QTable!!!
          this.$emit(
            "selection-changed-table-bauinformations-profile-bauinformations-profil",
  ...
  watch: {
    selectedRowsTableBauinformationsProfile: {
      ...
      immediate: true,
      handler: function() {
        console.log("In watch für selected mit", this.selectedRowsTableBauinformationsProfile);
        this.internalSelectedRowsTableBauinformationsProfile =
          this.selectedRowsTableBauinformationsProfile !== null &&
          this.selectedRowsTableBauinformationsProfile !== undefined
            ? [this.selectedRowsTableBauinformationsProfile]
            : [];
      }
    }
  }
...
<style lang="sass">
/*============================================================================================================================*/
/* Aktueller q-table Aufbau für Tabelle Bauinformationssprofile: */
/* Header: */
...
.stickyColumnsTable1HeaderLineBauinformationssProfile
  /*----------------------------------------------------------------------------------------------------------------------------*/
