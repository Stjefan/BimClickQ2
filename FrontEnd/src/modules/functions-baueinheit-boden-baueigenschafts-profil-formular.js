// functions-baueinheit-boden-baueigenschafts-profil-formular.js
// JS-Modul, das computed Properties und Methods als Functions exportiert
// Dieses JS-Modul ist ein Pendant zum Layout für das Profil "Baueinheit > Boden" (layout-baueinheit-boden-baueigenschafts-profil-formular.js)
...
//********************************************************************************************************************************** */
// HINwEIS: Enthält aktuell nur Vorlagen für Testfunktionen. Analog werden für "Baueinheit > Boden" KEINE profilspezifischen
//          Methoden benötigt, die dann in mounted() von layout-gesteuertes-generisches-baueigenschafts-profil-formular.js aus dem
//          Modul hier geladen werden könnten! Der Grund dafür ist, dass alle benötigten Methoden als generische, profilübergreifende
...
//********************************************************************************************************************************** */
import LayoutBaueinheitBodenBaueigenschaftsProfilFormular from "./layout-baueinheit-boden-baueigenschafts-profil-formular";
// Das Layout File für dieses Profil - wird benötigt z.B. zum Zugriff auf den Index von Baueigenschaften im Profil Model
const FunctionsBaueinheitBodenBaueigenschaftsProfilFormular = function(self) {
  ...
  var layoutBaueinheitBodenBaueigenschaftsProfilFormular = LayoutBaueinheitBodenBaueigenschaftsProfilFormular; // Das Layout für dieses Profil in Variable
  //-----------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  */
  ...
  that.testSetMethod = function(aufzaehlungsWert, baueigenschaft) {
    ...
    // Test 2 und 3 in layout-gesteuertes-generisches-baueigenschafts-profil-formular.vue
  ...
  that.sollIstWertePaarSetMethod = function(baueigenschaft1, baueigenschaft2) {
    ...
    // Einstweilen nur für Aufzählungwerte und Kennwerte (TODO: Wertetyp abfragen und für weitere Wertetypen verallgemeinern)
    ...
    // Zuerst prüfen, ob der Istwert nicht null ist, und wenn ja, diesen unverändert returnen
    if (
    ...
    } // TODO XXXXXX: Hier vollständige null-Prüfung für Aufzählungswerte (können Arrays oder Strings sein!)
    if (
      baueigenschaft2.typbezogeneBaueigenschaft.werteTyp === "Kennwert" &&
      baueigenschaft2.kennWert1 != null
    ) {
      return baueigenschaft2.kennWert1;
    }
    // ACHTUNG: In obigen Fällen muss unbedingt baueigenschaft2.aufzaehlungsWert bzw. .kennWert1 (unverändert) RETURNED werden, sonst funktioniert es nicht!!!
    // TODO XXXXXX: Hier alle Wertetypen unterstützen
    // Der Istwert ist hier nun null - nun diesen mit dem Sollwert befüllen und das Ergebnis returnen:
    let result = null;
    if (
      baueigenschaft2.typbezogeneBaueigenschaft.werteTyp === "Aufzählungswert"
    ) {
      result = _self.$set(
        // Testhalber Baueigenschaft updaten im Profil Model, Vue $set returns the set value
    ...
    if (baueigenschaft2.typbezogeneBaueigenschaft.werteTyp === "Kennwert") {
      result = _self.$set(
        // Testhalber Baueigenschaft updaten im Profil Model, Vue $set returns the set value
        baueigenschaft2, // Diese BEg mit diesem Testwert aktualisieren
      ...
      console.log("result: ", result);
      console.log("baueigenschaft2.kennWert1: ", baueigenschaft2.kennWert1)
      return baueigenschaft2.kennWert1
    }
  };
  //-----------------------------------------------------------------------------------------------------------------------------------------------------
  that.testGeneralSetMethod = function(baueigenschaft1, baueigenschaft2) {
    ...
    // Test 4 in layout-gesteuertes-generisches-baueigenschafts-profil-formular.vue
    // Parameter: Zwei Baueigenschafen aus dem Profil Model
  ...
  that.testSpecificSetMethod = function(baueigenschaft) {
    ...
    // Test 5 in layout-gesteuertes-generisches-baueigenschafts-profil-formular.vue
  ...
  that.testSpecificReadonlyMethod = function(baueigenschaft) {
    ...
    // Test 8 in layout-gesteuertes-generisches-baueigenschafts-profil-formular.vue
    // Parameter: Baueigenschaft aus dem Profil Model
  ...
  that.testGeneralReadonlyMethod = function(
    ...
    // Test 9 in layout-gesteuertes-generisches-baueigenschafts-profil-formular.vue
    // Parameter:
  ...
  that.testGeneralInitializeMethod = function(
    ...
    //========================================================================================================================================================
    // HINWEIS: Diese Methode wurde final durch eine generalisierte Funktion ersetzt:
    // layout-gesteuertes-generisches-baueigenschafts-profil-formular.initializeIstWerteMitSollWerten()
    // Diese initialisiert das Profil Model für solche Baueigenschaften, die im Layout eine Prop "initializeMitSollWert" haben, zusammen mit
    ...
    // Diese Funktion ist für alle Profile/Layouts identisch, und daher in layout-gesteuertes-generisches-baueigenschafts-profil-formular!
    // ACHTUNG: Die Alternative, diese Initialisierung durch Funktionen, die aus dem Template aufgerufen werden und :value oder v-model updaten, durchzuführen,
    // funktioniert nicht!!! Der Grund dafür ist, dass sich die Initialisierung im Fall null und die Eingabe in das Eingabefeld durch den Benutzer ins Gehege
    // kommen! Für Readonly-Felder funktioniert es dagegen!
    //========================================================================================================================================================
    // Generelle Methode für Initialisierung von Eingabefeldern - ist hier als Funktion mit Parametern definiert
    // Test 10 in layout-gesteuertes-generisches-baueigenschafts-profil-formular.vue
    // Parameter:
...
export default FunctionsBaueinheitBodenBaueigenschaftsProfilFormular;
// ACHTUNG: Hier statt module.exports = FunctionsBaueinheitBodenBaueigenschaftsProfilFormular;
